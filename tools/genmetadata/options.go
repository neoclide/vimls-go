...
func writeOptionSetOracle(path string, options []option) error {
	...
	output.WriteString("      endif\n")
	output.WriteString("    endif\n")
	output.WriteString("    if actual !=# a:expected\n")
	output.WriteString("      call add(s:option_set_failures, a:name .. ' completion [' .. a:source .. ']: expected ' .. string(a:expected) .. ', got ' .. string(actual))\n")
	output.WriteString("    endif\n")
	output.WriteString("  catch\n")
