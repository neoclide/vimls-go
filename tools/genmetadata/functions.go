...
func generateFunctions(vimRoot, output string) error {
	...
	if err := addFunctionDocumentation(vimRoot, functions); err != nil {
		return fmt.Errorf("read builtin documentation: %w", err)
	}
...
func addFunctionDocumentation(root string, functions []builtin) error {
	...
	documentation := make(map[string]vimhelp.Documentation, len(functions))
	...
	for index := range functions {
		...
		if doc.Markdown == "" || doc.Source == "" {
			return fmt.Errorf("documentation for %s() is empty", functions[index].Name)
		}
		functions[index].Documentation = doc.Markdown
		functions[index].DocumentationSource = doc.Source
	}
	return nil
}
...
func writeFunctions(path string, functions []builtin) error {
	...
	for _, function := range functions {
		...
		fmt.Fprintf(&generated, ", Documentation: %q, DocumentationSource: %q", function.Documentation, function.DocumentationSource)
		fmt.Fprintln(&generated, "},")
	}
	fmt.Fprintln(&generated, "}")
