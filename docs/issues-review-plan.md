...
| H-2 | 嵌套切片共享确实违反副本所有权；未发现生产调用者写入导致的现行竞态 | P1，补全副本隔离 |
...
| L-10 | tidy 确认可删除大量校验和，并修正 uri 的直接依赖声明 | P2，独立清理 |
...
计划：让枚举和按名查询的返回值都拥有自己的嵌套切片，保留 nil 的语义。
增加测试覆盖元素写入、重新切片/append 后的隔离及第二次查询不受影响。
...
L-10：本次只运行 `go mod tidy -diff`，没有修改模块文件。
结果将 go.sum 从 231 行降至 10 行，并把实际直接导入的 `go.lsp.dev/uri` 移到直接依赖块，依赖版本不变。
大量条目与 protocol 模块中的 tool 依赖对应，不能仅因没在根 go.mod 中看到名字就手工删除校验和。
计划由 tidy 生成精确清理，再验证模块可只读构建、测试与工具目标，不升级依赖、不改变工具链版本。
...
实施范围为 `internal/parsecmd`、`cmd/vimparse/main.go` 及对应文档，其他编号的生产修复未实施。
