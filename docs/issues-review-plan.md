...
输入是用户提供的未跟踪文件 `issues.md`，原文保持不变，不纳入提交。
后续用户选择已确认：实施 M-8 的基础输入防护及 stdin 支持，不新增 L-8 的冲突方言修饰符提示。
已确认处理的 13 个编号均已完成，分别形成 12 个本地修复提交（H-3/H-4 合并）。驳回和暂缓项保持下表决定。
语言证据读取 `/Users/chemzqm/lib/vim` 的 **v9.2.1015** 对象；该 checkout 的未跟踪文件保持原状。
原始复核未运行 Vim 脚本；后续仅运行仓库中专门编写的干净 Vim oracle，版本、断言和退出状态见各项验证记录。

## 结论与优先顺序

原报告的“7 项高危”不能直接作为修复清单。部分问题成立，但严重度、触发条件或修复方案有误；有些建议会改变现有正确行为。

原定优先处理 H-5 和 L-6：两处都存在可用很小输入触发的重复扫描，其中 L-6 并非原报告所称的最多两遍。
...
| 原编号 | 复核结论 | 处理状态 |
| --- | --- | --- |
| H-1 | CRLF 中间偏移确实拒绝转换，但符合现有严格坐标约定；未复现诊断全部丢失 | 保留严格转换；已随 M-4 加强边界验收 |
| H-2 | 嵌套切片共享确实违反副本所有权；未发现生产调用者写入导致的现行竞态 | 已修复，枚举和查询均深拷贝 |
| H-3 | 线性查询成立，“每个标识符数百次比较”过度概括 | 已与 H-4 合并优化 |
| H-4 | 不必要拷贝成立；固定 6 次分配与表大小数据不准确 | 已保留安全 API 并增加轻量查询 |
| H-5 | 自动续行有多处累计文本重扫、重拷贝 | 已修复，追加收集与延迟表达式解析 |
| H-6 | 忽略了服务层安装身份检查；worker pool 并不直接乱序写入在线索引 | 驳回所述缺陷，不添加重复版本体系 |
| H-7 | 路径映射接受不安全字符串；实际利用路径未证明 | 已修复，收紧静态 autoload 路径边界 |
| M-1 | 未响应配置请求可在活跃连接中长期积压，通知不受请求数量上限保护 | 已修复，超时、取代取消、生命周期绑定 |
| M-2 | 独立 Stream.Read 无法取消阻塞读取；生产 Run 已有取消与关闭路径 | 已补充所有权契约和真实传输取消验证 |
| M-3 | 固定版本 Vim 允许循环导入，不应按环路统一报错 | 驳回通用循环诊断 |
| M-4 | 独立 CR 不被 text 和 parser 正确切行，违反 LSP 文本约定 | 已修复，贯通 text/syntax/server |
| M-5 | 中间快照重复哈希成立；后续编辑仍需当前行索引 | 已优化，仅最终快照哈希 |
| M-6 | 子串持有源字符串成立，但当前仅用于生成器，不是服务器长期泄漏 | 暂缓，先有 retained heap 收益证据 |
| M-7 | 空文档导致整体失败成立，但这是生成器的严格完整性校验 | 保留，不静默跳过或向前误合并 |
| M-8 | 输入无界与特殊文件是加固点；用户选择基础防护和 stdin | 已修复，提交 `c8681eb` |
| M-9 | 三种坐标各有职责；未证明支持平台/输入上的溢出 | 保留分层，转换 helper 仅随相关工作收敛 |
| L-1 | 先取 200 个目录项会漏掉后面的匹配项 | 已修复，提交 `34fe3e6` |
...
| L-6 | 多个未闭合 heredoc 反复扫描后缀，存在平方增长 | 已修复，共享精确行索引并保留恢复规则 |
...
已保持严格转换；M-4 的端到端验收覆盖换行附近的诊断、符号和编辑范围。
...
已实施上述索引、枚举顺序预计算、`IsOption`/`IsFunction` 及生产调用迁移。事件的 ASCII 查询使用索引，非 ASCII 输入保留 EqualFold 回退。
`LookupOption` 与 `LookupFunction` 继续返回嵌套副本；本项不增加返回全局指针的接口。
元数据与 analysis 包测试、server 内建语义 token/方法补全定向测试、本地 `gopls check` 通过。
`metadata_bench_test.go` 覆盖命中、未命中、短名、sigil、事件大小写、枚举与初始化；`BenchmarkOptionHeavyAnalysis` 使用 100 组真实 set/option/function/variable 表达式。
同一 Intel i7-9750H/darwin-amd64、3 次运行中位数，对比 `620785f` 与本项工作区；查询 `-benchtime=50ms`，analysis `100ms`：

| 场景 | ns/op，前 → 后 | B/op，前 → 后 | allocs/op，前 → 后 |
| --- | --- | --- | --- |
| LookupFunction(map) | 2568 → 61.48 | 32 → 32 | 1 → 1 |
| LookupFunction(xor) | 3173 → 53.93 | 32 → 32 | 1 → 1 |
| LookupOption(number) | 8703 → 213.0 | 184 → 184 | 3 → 3 |
| LookupOption(nu) | 21826 → 210.9 | 184 → 184 | 3 → 3 |
| LookupOption(missing_option) | 24839 → 46.06 | 0 → 0 | 0 → 0 |
| number 存在性查询 | 9074 → 11.26 | 184 → 0 | 3 → 0 |
| LookupVariable(v:version) | 166.7 → 15.21 | 0 → 0 | 0 → 0 |
| LookupAutocmdEvent(WinScrolled) | 530.1 → 27.86 | 0 → 0 | 0 → 0 |
| Options() | 186201 → 151566 | 362824 → 362354 | 1675 → 1672 |
| Variables() | 14694 → 1798 | 9624 → 9472 | 4 → 1 |
| OptionValues(ambiwidth) | 6346 → 53.75 | 280 → 32 | 6 → 1 |
| 100 组 option-heavy analysis | 28752556 → 3734911 | 2117936 → 1994752 | 15539 → 13339 |

四张索引与两份排序下标的一次性构建中位数为 80896 ns、103112 B、151 次分配。
结果仅对应本次可复现工作负载，不代表所有文件的端到端加速比例。

### H-5：自动续行需要整体消除累计工作

修复前，[`scanner.go`](../internal/syntax/scanner.go) 自动续行分支每次调用 `scanVim9CommandArgument` 重新扫描完整逻辑参数，
...
已实施：逻辑文本使用追加式 `strings.Builder`，源片段映射改为二分查找。
只在实际自动续行时分配收集状态，增量保留引号、注释、转义、括号、类型尖括号、三元表达式和 lambda 上下文；
扫描位置仍相对完整逻辑参数，保留跨物理行的 lookbehind。插值字符串也保留扫描进度。
表达式 AST 延迟到收集结束或潜在命令分隔符处构建；类型声明和函数签名不会每行重新扫描完整前缀。
保留显式续行、前导运算符、bar、注释和不完整命令恢复，不增加最大续行数。
为保留错误恢复，潜在分隔符和 `is` 等歧义词尾仍交给完整语法探针；这里不声称任意错误输入的整个 parser 都有线性复杂度。

语法包测试及本地 `gopls check` 通过。以 `da2814a` 独立 CLI 为基线，3141 个组合输入、现有语法测试字符串及 oracle 输入的公开 AST JSON 完全一致。
新增长列表/注释/分隔符/三种换行测试，以及增量扫描状态、插值状态与映射前缀不可变性测试。
新增续行 oracle 在固定 Vim 9.2 patch 1–1015 下通过：exit_status=0、`v:errors=[]`、`:messages` 为空。

同机 `BenchmarkParseAutomaticContinuation`、`-benchtime=1x -count=3`，以下均为 1024 次续行的中位数：

| 输入形态 | 修复前时间 | 修复后时间 | 修复前分配 | 修复后分配 |
| --- | ---: | ---: | ---: | ---: |
| 数值列表 | 120.31 ms | 1.25 ms | 110.56 MB | 0.97 MB |
| 含 bar/hash 的字符串列表 | 148.55 ms | 1.30 ms | 124.97 MB | 1.00 MB |
| 前导运算符 | 201.67 ms | 2.27 ms | 201.92 MB | 1.44 MB |
| 尾部运算符 | 204.91 ms | 2.39 ms | 203.12 MB | 1.44 MB |
| 方法链 | 364.26 ms | 4.04 ms | 211.20 MB | 1.45 MB |
| 函数签名 | 106.59 ms | 2.28 ms | 24.73 MB | 1.07 MB |
| for 列表 | 119.40 ms | 1.69 ms | 110.56 MB | 0.97 MB |
| block lambda | 1692.62 ms | 6.37 ms | 1909.81 MB | 4.81 MB |
| 带 bar 的 block lambda | 4407.78 ms | 12.68 ms | 4270.03 MB | 10.26 MB |
| 三元表达式链 | 577.53 ms | 5.22 ms | 407.91 MB | 2.41 MB |
| 嵌套类型声明 | 302.77 ms | 4.00 ms | 125.66 MB | 1.13 MB |

列表 N=128/256/512/1024 时，修复后时间为 0.21/0.35/0.64/1.25 ms，分配为 0.13/0.24/0.44/0.97 MB。
普通文件的独立对比没有额外分配增长：1 MiB Legacy 12.19→12.79 ms、Vim9 22.02→20.77 ms；
100 KiB Legacy 1.15→1.43 ms、Vim9 2.42→1.87 ms。64 个单行表达式命令 Legacy 0.45→0.47 ms、Vim9 0.73→0.70 ms。
小样本时间有波动，不据此宣称普通脚本也得到相同比例加速。
...
已实施：在 autoload 专用入口拒绝路径分隔符、NUL、行分隔符和 `.`/`..` 成分；保证映射结果仍在 `autoload/` 子目录。
将不可信/动态名字视为无法静态解析，不额外制造 Vim 错误诊断。
固定版本 `src/eval.c:eval_isnamec` 的普通名字扫描接受 ASCII、冒号和 `#`，`get_name_len` 另有动态名字展开路径；
`src/scriptfile.c:autoload_name` 仅去除 `g:` 并替换 `#`。因此路径入口保留 Unicode 和空段，不宣称它们是所有上下文中的合法标识符。
workspace/server 定向测试通过，覆盖普通/多级 autoload、目录穿越、Windows 分隔符、既有符号链接边界及索引导航隔离。
本地 `gopls check` 无诊断；此项依据 pinned 源码，无新增运行时语法规则或 Vim oracle 执行。
...
已实施：使用 text 包内部的短生命周期编辑状态，保留当前文本和需要的行索引，所有变化成功后才计算最终 ContentID 并产生不可变 Snapshot。
...
`go test -count=1 ./internal/text` 和本地 `gopls check` 通过。新增批量测试包含 BOM、组合字符、astral 字符、CRLF/LF、增删行和失败后原快照不变。
`BenchmarkApplyChangesBatch` 在 Intel i7-9750H/darwin-amd64、4096 行文本、`-benchtime=100ms -count=3` 下对比 `8fc0ff2` 与本项工作区：

| 场景 | ns/op 中位数，前 → 后 | B/op，前 → 后 | allocs/op，前 → 后 |
| --- | --- | --- | --- |
| 零变化 | 289295 → 43.99 | 357729 → 112 | 18 → 1 |
| 单次小编辑 | 303687 → 425458 | 456036 → 456039 | 20 → 20 |
| 32 次小编辑 | 10103323 → 3356181 | 14593155 → 11542222 | 640 → 578 |
| 32 次相同内容全量替换 | 9168507 → 91.88 | 11447333 → 112 | 576 → 1 |

这是有界局部基准，单次小编辑的分配未改善、该轮耗时反而上升；不声称所有编辑加速。
连续范围编辑仍复制字符串并重建行索引，最坏复杂度仍可能 O(KN)。全量替换不再建立用不到的中间索引。
...
已实施：首次遇到恢复候选行时，延迟建立同一源文件的精确物理行索引，记录各行内容的最后位置。
只需证明后面是否仍存在合法结束标记；存在则继续正常扫描，不存在则立即在原恢复行继续解析，无需先扫描到 EOF 再回退。
trim 模式遵循命令的精确缩进，索引兼容未缩进结束标记；不会为每个不同 marker 重扫全文。
...
语法包测试、本地 `gopls check` 通过；与 `902d6b4` 的独立 CLI 对比中，607 个方言/换行/trim/eval/函数/命令块组合及现有 oracle 输入的公开 AST JSON 完全一致。
新增测试覆盖 512 个连续不同缺失标记、1500 段之后的完整结束标记和精确缩进/位置规则。
新增 pinned heredoc oracle 在 Vim 9.2 patch 1–1015 下通过：exit_status=0、`v:errors=[]`、`:messages` 为空。
有界 CPU profile 还定位到没有 `|` 时的不必要命令块历史扫描，已按实际分隔符条件延迟；主扫描循环增量维护命令块深度。

`BenchmarkParseMissingHeredocs -benchtime=1x -count=3`，同一 Intel i7-9750H/darwin-amd64，毫秒中位数：

| 缺失标记数 | 相同 marker，前 → 后 | 不同 marker，前 → 后 |
| --- | --- | --- |
| 256 | 3.74 → 0.83 | 3.73 → 0.64 |
| 512 | 12.22 → 1.33 | 11.85 → 1.12 |
| 1024 | 47.38 → 3.75 | 43.70 → 3.31 |
| 2048 | 201.37 → 6.43 | 156.73 → 6.70 |

2048 个不同 marker 的分配量从约 6.03 MB 增至 6.25 MB，用于一次性精确行索引；相同 marker 仅增加约 272 B。
既有 `BenchmarkParseLargeFile` 的 1 MiB Legacy/Vim9 输入中位数从 43.30/28.85 ms 变为 15.39/21.98 ms；100 KiB Vim9 从 2.23 ms 到 2.37 ms，未声称每种输入都加速。
...
上述批次和 M-2 的传输验证均已完成；M-8 按用户选择实施基础输入防护及 stdin，L-8 保持现状。
...
不默认运行 race、coverage 或广泛 fuzz。影响支持行为的实施批次需同步受影响的公共文档与 roadmap；已影响支持行为的条目同步更新了对应公共契约与 roadmap。
...
| 范围 | 本地提交 |
| --- | --- |
| M-8 CLI 输入保护与 stdin | `c8681eb` |
| H-2 元数据副本所有权 | `95bd3e0` |
| L-10 模块清理 | `03a4311` |
| L-1 import 前缀过滤 | `34fe3e6` |
| H-7 autoload 路径校验 | `8fc0ff2` |
| M-5 最终快照哈希 | `376dec7` |
| M-1 配置请求生命周期 | `84ce8e9` |
| M-2 传输取消与所有权验证 | `620785f` |
| H-3/H-4 元数据索引与轻量查询 | `c154314` |
| M-4/H-1 换行支持与严格坐标边界 | `902d6b4` |
| L-6 未闭合 heredoc 扫描 | `da2814a` |
| H-5 自动续行收集 | `a174678` |

修复均为本地提交，没有推送、打标签或发布。原始输入 `issues.md` 继续保留为未跟踪文件。

## 最终集成验证

2026-09-07，在修复提交 `a174678` 上统一格式化全部改动 Go 文件（没有产生额外代码差异），随后一次完整执行：
...
全部通过；server、syntax、integration 及仓库 Go 工具均包含在全量测试中。
`make` 以 `-mod=readonly` 构建 `bin/vimls` 和 `bin/vimparse`。
H-5、L-6 和 CR 换行的固定版本 oracle 与差分/性能证据分别记录在对应条目；没有运行 race、coverage 或广泛 fuzz 活动。
此后仅更新本汇总及 changelog，完成内容、相对链接与 `git diff --check` 检查，不重复运行已通过的 Go gate。
