...
func TestCancelledRequestMatrix(t *testing.T) {
	...
	_, err = s.CodeLens(ctx, &protocol.CodeLensParams{TextDocument: protocol.TextDocumentIdentifier{URI: u}})
	mustCancelled("code lens", err)
	_, err = s.CodeLensResolve(ctx, &protocol.CodeLens{})
	mustCancelled("code lens resolve", err)
